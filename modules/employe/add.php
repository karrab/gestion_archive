...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            db()->prepare('INSERT INTO employe (matricule, nom, renom, service_id, mail, tel1, tel2, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
                ->execute([$matricule, $nom, $renom, $serviceId, $mail, $tel1, $tel2, $notes]);
            log_historique('ajout', 'employe', "Ajout de l'employé: {$nom} {$renom}");
...
?>
<h4 class="mb-3"><i class="bi bi-plus-lg"></i> إضافة موظف</h4>
    ...
    <form method="post">
      ...
      <div class="row">
        <div class="col-md-4 mb-3">
          ...
          <input type="text" name="matricule" class="form-control" required value="<?= e($_POST['matricule'] ?? '') ?>">
        </div>
        <div class="col-md-4 mb-3">
        ...
        <div class="col-md-4 mb-3">
          ...
          <input type="text" name="renom" class="form-control" required value="<?= e($_POST['renom'] ?? '') ?>">
        </div>
      </div>
      <div class="mb-3">
        ...
        <select name="service_id" class="form-select select2" required>
          ...
          <?php foreach ($services as $s): ?>
            <option value="<?= $s['id'] ?>" <?= ($_POST['service_id'] ?? '') == $s['id'] ? 'selected' : '' ?>><?= e($s['nom']) ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      <div class="row">
        <div class="col-md-4 mb-3">
          ...
          <input type="email" name="mail" class="form-control" value="<?= e($_POST['mail'] ?? '') ?>">
        </div>
        <div class="col-md-4 mb-3">
          <label class="form-label">الهاتف 1</label>
          <input type="text" name="tel1" class="form-control" value="<?= e($_POST['tel1'] ?? '') ?>">
        </div>
        <div class="col-md-4 mb-3">
          <label class="form-label">الهاتف 2</label>
          <input type="text" name="tel2" class="form-control" value="<?= e($_POST['tel2'] ?? '') ?>">
        </div>
      </div>
      <div class="mb-3">
