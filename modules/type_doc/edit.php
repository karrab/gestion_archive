...
$stmt = db()->prepare('SELECT * FROM type_doc WHERE id_typ = ?');
$stmt->execute([$id]);
$item = $stmt->fetch();
if (!$item) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        db()->prepare('UPDATE type_doc SET description = ?, notes = ? WHERE id_typ = ?')->execute([$description, $notes, $id]);
        log_historique('modification', 'type_doc', "Modification #{$id}");
...
$pageTitle = 'تعديل نوع الوثيقة';
require __DIR__ . '/../../includes/layout_header.php';
?>
<h4 class="mb-3"><i class="bi bi-pencil"></i> تعديل نوع الوثيقة</h4>
