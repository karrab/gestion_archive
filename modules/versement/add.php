...
require_permission('versement.add');

$errors = [];
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    csrf_verify();
    $dateVers = trim($_POST['date_vers'] ?? '');
    ...
    if ($dateVers === '' || $serviceId <= 0 || $employeId <= 0) {
    ...
    if (!$errors) {
        ...
        try {
            ...
            $pdo->prepare(
                'INSERT INTO versement (date_vers, ref_vers, num_bordereau, nb_doc, nb_boite, metrage_lin,
                    service, employe, institution, responsable_reception)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            )->execute([$dateVers, $refVers, $numBordereau, $nbDoc, $nbBoite, $metrage, $serviceId, $employeId, $institutionId, $responsable]);
            $idVers = (int) $pdo->lastInsertId();
            ...
            $stmtMaj = $pdo->prepare("UPDATE archive SET etat_archive = 'Versé' WHERE id = ?");
            ...
            foreach ($lignes as $l) {
                ...
                $stmtLigne->execute([
                    ...
                    $l['type_doc_id'] ?: null, $l['duree_conser'] ?? null, $l['sort_fin_doc_id'] ?: null, 'Versé',
                ]);
                if ($archiveId) {
            ...
            $pdo->commit();
            log_historique('ajout', 'versement', "Ajout du versement {$refVers} (BR: {$numBordereau})");
            flash_set('success', 'تمت الإضافة بنجاح.');
            redirect('view.php?id=' . $idVers);
        } catch (Throwable $e) {
            $pdo->rollBack();
...
$pageTitle = 'إضافة إيداع';
require __DIR__ . '/../../includes/layout_header.php';
?>
<h4 class="mb-3"><i class="bi bi-plus-lg"></i> إضافة إيداع</h4>
<?php foreach ($errors as $err): ?>
  <div class="alert alert-danger"><?= e($err) ?></div>
<?php endforeach; ?>
<div class="card mb-3">
  <div class="card-body">
    <form method="post" id="vers-form">
      <?= csrf_field() ?>
      <input type="hidden" name="lignes_json" id="lignes_json">
      <div class="row">
        <div class="col-md-2 mb-3">
          <label class="form-label">التاريخ</label>
          <input type="date" name="date_vers" class="form-control" required value="<?= e($_POST['date_vers'] ?? date('Y-m-d')) ?>">
        </div>
        <div class="col-md-2 mb-3">
...
<div class="card">
  <div class="card-body table-responsive">
    ...
    <button type="submit" form="vers-form" id="btn-save" class="btn btn-primary"><i class="bi bi-save"></i> حفظ</button>
...
document.getElementById('vers-form').addEventListener('submit', function () {
  document.getElementById('lignes_json').value = JSON.stringify(lignes);
});
</script>
