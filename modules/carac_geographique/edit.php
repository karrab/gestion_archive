...
$stmt = db()->prepare('SELECT * FROM carac_geographique WHERE id_geo = ?');
$stmt->execute([$id]);
$item = $stmt->fetch();
if (!$item) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        db()->prepare('UPDATE carac_geographique SET description = ?, notes = ? WHERE id_geo = ?')->execute([$description, $notes, $id]);
        log_historique('modification', 'carac_geographique', "Modification #{$id}");
...
$pageTitle = 'تعديل الخاصية الجغرافية';
require __DIR__ . '/../../includes/layout_header.php';
?>
<h4 class="mb-3"><i class="bi bi-pencil"></i> تعديل الخاصية الجغرافية</h4>
