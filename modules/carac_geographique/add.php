...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        db()->prepare('INSERT INTO carac_geographique (description, notes) VALUES (?, ?)')->execute([$description, $notes]);
        log_historique('ajout', 'carac_geographique', "Ajout: {$description}");
        flash_set('success', 'تمت الإضافة بنجاح.');
        redirect('list.php');
    }
}

$pageTitle = 'إضافة الخاصية الجغرافية';
require __DIR__ . '/../../includes/layout_header.php';
?>
<h4 class="mb-3"><i class="bi bi-plus-lg"></i> إضافة الخاصية الجغرافية</h4>
