...
$pdo = db();
$stmt = $pdo->prepare('SELECT * FROM transfert WHERE id_trans = ?');
$stmt->execute([$id]);
$trans = $stmt->fetch();
if (!$trans) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_ligne') {
    ...
    if ($titre === '') {
        ...
        try {
            ...
            if ($archiveId) {
                $pdo->prepare("UPDATE archive SET etat_archive = 'Transféré' WHERE id = ?")->execute([$archiveId]);
            }
            recalculer_transfert($pdo, $id);
            $pdo->commit();
            log_historique('ajout', 'transfert', "Ajout d'une ligne au transfert #{$id}");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_ligne') {
    csrf_verify();
    $ligneId = (int) ($_POST['id_ligtrans'] ?? 0);
    $pdo->beginTransaction();
    try {
        $stmt = $pdo->prepare('SELECT archive_id FROM ligne_transfert WHERE id_ligtrans = ? AND id_trans = ?');
        $stmt->execute([$ligneId, $id]);
        $archiveId = $stmt->fetchColumn();

        $pdo->prepare('DELETE FROM ligne_transfert WHERE id_ligtrans = ? AND id_trans = ?')->execute([$ligneId, $id]);
        if ($archiveId) {
            $pdo->prepare("UPDATE archive SET etat_archive = 'Disponible' WHERE id = ?")->execute([$archiveId]);
        }
        recalculer_transfert($pdo, $id);
        $pdo->commit();
        log_historique('suppression', 'transfert', "Suppression d'une ligne du transfert #{$id}");
...
function recalculer_transfert(PDO $pdo, int $idTrans): void
{
    $stmt = $pdo->prepare('SELECT num_boite, metrage_lin FROM ligne_transfert WHERE id_trans = ?');
    $stmt->execute([$idTrans]);
    $rows = $stmt->fetchAll();
    $nbDoc = count($rows);
    $nbBoite = count(array_unique(array_filter(array_column($rows, 'num_boite'))));
    $pdo->prepare('UPDATE transfert SET nb_doc = ?, nb_boite = ? WHERE id_trans = ?')->execute([$nbDoc, $nbBoite, $idTrans]);
}

$lignes = $pdo->prepare('SELECT * FROM ligne_transfert WHERE id_trans = ? ORDER BY id_ligtrans');
$lignes->execute([$id]);
...
$pageTitle = 'وثائق النقل';
require __DIR__ . '/../../includes/layout_header.php';
?>
<h4 class="mb-3"><i class="bi bi-list"></i> وثائق النقل رقم <?= e($trans['num_bordereau']) ?></h4>
        ...
        <tr>
          ...
          <td>
            <form method="post" class="d-inline">
              ...
              <input type="hidden" name="id" value="<?= (int) $id ?>">
              <input type="hidden" name="id_ligtrans" value="<?= (int) $l['id_ligtrans'] ?>">
