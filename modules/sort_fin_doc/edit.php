...
$stmt = db()->prepare('SELECT * FROM sort_fin_doc WHERE id_sort = ?');
$stmt->execute([$id]);
$item = $stmt->fetch();
if (!$item) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        db()->prepare('UPDATE sort_fin_doc SET description = ?, notes = ? WHERE id_sort = ?')->execute([$description, $notes, $id]);
        log_historique('modification', 'sort_fin_doc', "Modification #{$id}");
...
$pageTitle = 'تعديل مصير الوثيقة';
require __DIR__ . '/../../includes/layout_header.php';
?>
<h4 class="mb-3"><i class="bi bi-pencil"></i> تعديل مصير الوثيقة</h4>
