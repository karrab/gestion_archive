...
$pdo = db();
$stmt = $pdo->prepare('SELECT * FROM elimination WHERE id_elm = ?');
$stmt->execute([$id]);
$elim = $stmt->fetch();
if (!$elim) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_ligne') {
    ...
    if ($titre === '') {
        ...
        try {
            ...
            if ($archiveId) {
                $pdo->prepare("UPDATE archive SET etat_archive = 'Eliminé' WHERE id = ?")->execute([$archiveId]);
            }
            recalculer_elimination($pdo, $id);
            $pdo->commit();
            log_historique('ajout', 'elimination', "Ajout d'une ligne à l'élimination #{$id}");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_ligne') {
    csrf_verify();
    $ligneId = (int) ($_POST['id_ligelm'] ?? 0);
    $pdo->beginTransaction();
    try {
        $stmt = $pdo->prepare('SELECT archive_id FROM ligne_elimination WHERE id_ligelm = ? AND id_elm = ?');
        $stmt->execute([$ligneId, $id]);
        $archiveId = $stmt->fetchColumn();

        $pdo->prepare('DELETE FROM ligne_elimination WHERE id_ligelm = ? AND id_elm = ?')->execute([$ligneId, $id]);
        if ($archiveId) {
            $pdo->prepare("UPDATE archive SET etat_archive = 'Disponible' WHERE id = ?")->execute([$archiveId]);
        }
        recalculer_elimination($pdo, $id);
        $pdo->commit();
        log_historique('suppression', 'elimination', "Suppression d'une ligne de l'élimination #{$id}");
...
function recalculer_elimination(PDO $pdo, int $idElm): void
{
    $stmt = $pdo->prepare('SELECT num_boite FROM ligne_elimination WHERE id_elm = ?');
    $stmt->execute([$idElm]);
    $rows = $stmt->fetchAll();
    $nbDoc = count($rows);
    $nbBoite = count(array_unique(array_filter(array_column($rows, 'num_boite'))));
    $pdo->prepare('UPDATE elimination SET nb_doc = ?, nb_boite = ? WHERE id_elm = ?')->execute([$nbDoc, $nbBoite, $idElm]);
}

$lignes = $pdo->prepare('SELECT * FROM ligne_elimination WHERE id_elm = ? ORDER BY id_ligelm');
$lignes->execute([$id]);
...
$pageTitle = 'وثائق الإتلاف';
require __DIR__ . '/../../includes/layout_header.php';
?>
<h4 class="mb-3"><i class="bi bi-list"></i> وثائق الإتلاف رقم <?= e($elim['num_bordereau']) ?></h4>
        ...
        <tr>
          ...
          <td>
            <form method="post" class="d-inline">
              ...
              <input type="hidden" name="id" value="<?= (int) $id ?>">
              <input type="hidden" name="id_ligelm" value="<?= (int) $l['id_ligelm'] ?>">
