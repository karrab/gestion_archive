...
$selectFromSql = "SELECT id_dept, numero, description, notes FROM depot";

if ($perPage === 'all') {
    $whereSql = $where ? 'WHERE ' . implode(' AND ', $where) : '';
    $stmt = $pdo->prepare("{$selectFromSql} {$whereSql} ORDER BY {$sort} {$dir}, id_dept {$dir}");
    $stmt->execute($params);
    $items = $stmt->fetchAll();
    $hasMore = false;
} else {
    $result = keyset_paginate($pdo, $selectFromSql, $where, $params, "{$sort}", $dir, 'id_dept', $cursorVal, $cursorId, (int) $perPage);
    $items = $result['items'];
      ...
      <?php foreach ($items as $it): ?>
        <tr>
          ...
          <td>
            ...
            <?php if (has_permission('reference.edit')): ?><a href="edit.php?id=<?= $it['id_dept'] ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a><?php endif; ?>
...
<div class="d-flex justify-content-between align-items-center mt-3">
  ...
  <div>
    <?php if ($hasMore && $items): ?>
      <?php $last = end($items); ?>
      <a href="<?= e(pagination_url(['cursor_val' => $last[$sort], 'cursor_id' => $last['id_dept']])) ?>" class="btn btn-sm btn-outline-primary">الصفحة التالية <i class="bi bi-chevron-left"></i></a>
    <?php endif; ?>
  </div>
</div>
<?php endif; ?>
