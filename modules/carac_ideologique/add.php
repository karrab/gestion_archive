...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        db()->prepare('INSERT INTO carac_ideologique (description, notes) VALUES (?, ?)')->execute([$description, $notes]);
        log_historique('ajout', 'carac_ideologique', "Ajout: {$description}");
        flash_set('success', 'تمت الإضافة بنجاح.');
        redirect('list.php');
    }
}

$pageTitle = 'إضافة الخاصية الإيديولوجية';
require __DIR__ . '/../../includes/layout_header.php';
?>
<h4 class="mb-3"><i class="bi bi-plus-lg"></i> إضافة الخاصية الإيديولوجية</h4>
